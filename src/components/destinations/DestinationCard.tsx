...
const DestinationCard: React.FC<DestinationCardProps> = ({ destination }) => {
  ...
  const handleImageNav = useCallback((e: React.MouseEvent, direction: 'prev' | 'next') => {
    e.stopPropagation();
    if (direction === 'prev') {
        ...
        >
          ...
          <div className="absolute top-3 right-3">
            <Heart
              ...
              className={`h-7 w-7 ${isFavorited ? 'fill-red-500' : 'fill-none'
                } stroke-white cursor-pointer transition`}
            />
          </div>
        </div>
