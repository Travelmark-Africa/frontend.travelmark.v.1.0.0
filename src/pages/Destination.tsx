...
const Destination = () => {
  ...
  const averageRating = destination.reviews.length > 0
          ...
          <div className="fixed inset-0 bg-black bg-opacity-90 z-50 flex items-center justify-center p-4">
            ...
            <Carousel
              ...
              renderArrowPrev={(onClickHandler, hasPrev) =>
                hasPrev && (
                  <button onClick={onClickHandler} className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white/20 p-2 rounded-r-md hover:bg-white/40 transition">
              ...
              renderArrowNext={(onClickHandler, hasNext) =>
                hasNext && (
                  <button onClick={onClickHandler} className="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white/20 p-2 rounded-l-md hover:bg-white/40 transition">
              ...
              {displayImages.slice(1).map((image, index) => (
                <div key={index} className="relative overflow-hidden rounded-md group h-[290px]">
                  ...
                  {index === displayImages.length - 2 && (
                    <div onClick={() => setShowAllImages(true)} className="absolute inset-0 bg-black/50 flex items-center justify-center cursor-pointer hover:bg-black/70 transition-all duration-300">
            ...
            <div className="bg-white border rounded-lg p-6 shadow-md sticky top-4">
              ...
              <form onSubmit={handleSendEnquiry} className="space-y-4">
                ...
                <input
                  ...
                  required
                />
                <div onClick={() => setShowDatePicker(true)} className="w-full p-2 border rounded-md flex items-center cursor-pointer">
                ...
                <div className="flex items-center border rounded-md p-2">
                  ...
                  <button type="button" className="px-2 py-1 bg-gray-100 rounded-md" onClick={() => setTravelerCount(Math.max(1, travelerCount - 1))}>-</button>
                  <span className="mx-4">{travelerCount}</span>
                  <button type="button" className="px-2 py-1 bg-gray-100 rounded-md" onClick={() => setTravelerCount(travelerCount + 1)}>+</button>
                </div>
                <textarea
                  name="message"
                  placeholder="Message"
