      ...
      <nav className="flex-1 space-y-2 px-2">
        {(navLinks as NavLink[]).map((link: NavLink) => (
          <Link
            key={link.label}
            to={link.href}
            className={`group flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all duration-500 ease-in-out ${isActivePath(link.href) ? "text-secondary" : "text-gray-600 hover:text-secondary"
              }`}
          >
            <span
              className={`transition-colors duration-200 ${isActivePath(link.href) ? "text-secondary" : "text-gray-500 group-hover:text-secondary"
                }`}
            >
              {link.icon}
            </span>
